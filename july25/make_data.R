...
## File description -------------------------------------------------------------
## Make data for the hierarchical clustering visualization with treatment and
## control comparisons.
##
...
phy_df <- rbind(
  ...
  mutate(id = revalue(as.character(id), mapping))

cat(sprintf("var tree = %s;", jsonlite::toJSON(phy_df)), file = "~/Desktop/100_days/july25/tree.js")
...
cat(sprintf("var data = %s;", jsonlite::toJSON(js_data)), file = "~/Desktop/100_days/july25/data.js")

ts_data <- js_data %>%
  dlply(.(fill, column, group), function(z) {
    z
  })
names(ts_data) <- NULL
cat(sprintf("var ts_data = %s;", jsonlite::toJSON(ts_data)), file = "~/Desktop/100_days/july25/ts_data.js")
